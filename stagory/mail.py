...
def send_email(addr_to, msg_subj, msg_text, files):
    addr_from = "admin@24vgk.ru"  # Отправитель
    password = "V1qaz2wsx"  # Пароль

    msg = MIMEMultipart()  # Создаем сообщение
    msg['admin@24vgk.ru'] = addr_from  # Адресат
    msg['kalabin_as@cscentr.com'] = addr_to  # Получатель
    msg['TEST'] = msg_subj  # Тема сообщения

    body = msg_text  # Текст сообщения
    msg.attach(MIMEText(body, 'Проверка текста сообщения'))  # Добавляем в сообщение текст

    process_attachement(msg, files)

    # ======== Этот блок настраивается для каждого почтового провайдера отдельно ===============================================
    server = smtplib.SMTP_SSL('admin@24vgk.ru', 465)  # Создаем объект SMTP
...
addr_to = "admin@yandexvrn.ru"  # Получатель
